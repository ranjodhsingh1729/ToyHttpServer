...
int log_msg(LogLevel lg_lvl, char *fmtstr, ...) {
    ...
    int bytes_written = fprintf(g_cur_log_stream, "[%04d-%02d-%02d %02d:%02d:%02d] ",
        tm_info->tm_year + 1900, tm_info->tm_mon + 1, tm_info->tm_mday,
        tm_info->tm_hour, tm_info->tm_min, tm_info->tm_sec);

    switch (lg_lvl) {
        case LOG_LVL_DEBUG:   bytes_written += fprintf(g_cur_log_stream, "[DEBUG] "); break;
        case LOG_LVL_INFO:    bytes_written += fprintf(g_cur_log_stream, "[INFO] "); break;
        case LOG_LVL_WARN:    bytes_written += fprintf(g_cur_log_stream, "[WARN] "); break;
        case LOG_LVL_ERROR:   bytes_written += fprintf(g_cur_log_stream, "[ERROR] "); break;
        case LOG_LVL_CRITICAL:bytes_written += fprintf(g_cur_log_stream, "[CRIT] "); break;
        default:              bytes_written += fprintf(g_cur_log_stream, "[???] "); break;
    }
