...
int main() {
  ...
  int opt = 1;
  if (setsockopt(server_sock_in_fd, SOL_SOCKET, SO_REUSEADDR, &opt,
                 sizeof(opt)) == -1 ||
      setsockopt(server_sock_in6_fd, SOL_SOCKET, SO_REUSEADDR, &opt,
                 sizeof(opt)) == -1) {
  ...
  if (bind(server_sock_in_fd, (struct sockaddr *)(&server_sockaddr_in),
  ...
  if (bind(server_sock_in6_fd, (struct sockaddr *)(&server_sockaddr_in6),
           sizeof(server_sockaddr_in6)) == -1) {
  ...
  log_info("Server listening on %s:%d (IPv4), [%s]:%d (IPv6)", IPV4_ADDR, PORT,
           IPV6_ADDR, PORT);
  ...
  while (!stop_server) {
    ...
    if (pollfds[0].revents & POLLIN) {
      client_sock_in_fd =
          accept(server_sock_in_fd, (struct sockaddr *)(&client_sockaddr_in),
      ...
      if (pthread_create(&thread, NULL, connection_handler,
                         &client_sock_in_fd) != 0) {
        log_error("Failed to create thread for IPv4 connection: %s",
                  strerror(errno));
    ...
    if (pollfds[1].revents & POLLIN) {
      client_sock_in6_fd =
          accept(server_sock_in6_fd, (struct sockaddr *)(&client_sockaddr_in6),
                 &client_sockaddr_in6_size);
      ...
      pthread_t thread;
      if (pthread_create(&thread, NULL, connection_handler,
                         &client_sock_in6_fd) != 0) {
        log_error("Failed to create thread for IPv6 connection: %s",
                  strerror(errno));
